...
data class NoteRange(val from: Note, val to: Note) {
    ...
    init {
        if (from >= to)
            throw IllegalArgumentException("Left border cannot be greater than or equal to the right")
    }

    /** Количество нот в диапазоне */
    val noteCount: Int
        get() = (to.octave * 7 - to.name.ordinal) - (from.octave * 7 - from.name.ordinal)
